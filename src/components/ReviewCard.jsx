...
function ReviewCard({ review, onUpdated }) {
  const { user } = useAuth();
    ...
    <div className="bg-gray-50 rounded-xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow mb-4">
      ...
      <div className="flex items-center gap-3 mb-3">
        <div className="w-10 h-10 bg-blue-100 text-blue-700 rounded-full flex items-center justify-center font-bold uppercase border border-blue-200">
          {/* Grabs first letter of name from nested User object */}
          {review.User?.name ? review.User.name.charAt(0) : "U"}
        </div>
        <div>
          <div className="font-bold text-gray-900 leading-tight">
            {/* Displays actual name from database */}
      ...
      {editing ? (
        <div className="mt-2 bg-white p-4 rounded-lg border border-gray-200">
          <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Update Rating</label>
          <select
            value={form.rating}
          ...
          <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Update Comment</label>
          <textarea
            value={form.comment}
          ...
          <div className="flex gap-2">
            <button onClick={handleSave} className="bg-green-600 hover:bg-green-700 text-white px-4 py-1 rounded font-medium">
              Save
            </button>
            <button onClick={() => setEditing(false)} className="bg-gray-200 text-gray-700 px-4 py-1 rounded">
              Cancel
            </button>
          </div>
        </div>
